...
export function RequestTimelineView({ request, tasks, zoom, onSelectTask }: RequestTimelineViewProps) {
  ...
  tasks
    ...
    .forEach((task) => {
      items.push({
        ...
        color: task.category === "Dump" || task.category === "Cache2Backend" ? "#b45309" : "#0f766e",
        meta: {
  ...
  return <TimelineChart title="请求生命周期与相关 UC task" items={items} zoom={zoom} onItemClick={onSelectTask} />;
}
